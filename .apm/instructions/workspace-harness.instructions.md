---
description: Workspace Harness operating boundaries and project isolation rules
applyTo: "**"
---
...
- If LeanCTX reports the Workspace as untrusted, the declared `.lean-ctx.toml` policy is not in effect and `owlspec` calls routed through LeanCTX are blocked. Report it and ask the operator to run `lean-ctx trust` in the Workspace root; it is pinned to the file's contents, so it is required again after every edit.
